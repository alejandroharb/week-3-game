...
// toDo:
// prevent word storing in userGuessArr after it's been chosen once
...
var gameStart = document.addEventListener('keyup', function(event){
	...
	if (event.keyCode == 32){ //space or enter start game
		alert("game begins!");

		document.removeEventListener('keyupd', gameStart);
		...
		console.log("Word Blank: " + blankWordDisplay);

		console.log("-------------------");
		var userGuessArr = []
		var wrongUserGuessArr = [];
			...
			if ((event.keyCode <=90) && (event.keyCode >= 65)) { // to detect only numbers
				...
				function checkLetter (checkArr,letter,correctBankArr) {
					for(var i = 0; i <checkArr.length; i++) {
						if(letter == checkArr[i]) {
							console.log("letter " + letter + " found")
							correctBankArr.push(letter);
						}
					}
				};
				checkLetter(randomWordArr, userGuess, userGuessArr);
				console.log(userGuessArr);

				console.log(userGuessArr.sort());
				console.log(randomWordArr.sort());
				...
				compareArrays(randomWordArr,userGuessArr);
			}
		});
	}
});
